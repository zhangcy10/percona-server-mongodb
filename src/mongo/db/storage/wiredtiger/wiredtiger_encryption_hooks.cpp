...
Status WiredTigerEncryptionHooksCBC::unprotectTmpData(
    ...
    *resultLen = 0;
    ...
    int decrypted_len = 0;
    if (1 != EVP_DecryptUpdate(ctx, out, &decrypted_len, in, inLen - _chksum_len))
        return handleCryptoErrors();
    *resultLen += decrypted_len;
    in += inLen - _chksum_len;
    inLen = _chksum_len;
...
Status WiredTigerEncryptionHooksGCM::protectTmpData(
    ...
    *resultLen += encrypted_len;
    ...
    // get the tag
    if(1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, _gcm_tag_len, out + *resultLen))
        return handleCryptoErrors();
...
Status WiredTigerEncryptionHooksGCM::unprotectTmpData(
    ...
    *resultLen = 0;
    ...
    int decrypted_len = 0;
    if (1 != EVP_DecryptUpdate(ctx, out, &decrypted_len, in, inLen - _gcm_tag_len))
        return handleCryptoErrors();
    *resultLen += decrypted_len;
    in += inLen - _gcm_tag_len;
    inLen = _gcm_tag_len;

    // Set expected tag value. Works in OpenSSL 1.0.1d and later
    if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, _gcm_tag_len, const_cast<uint8_t*>(in)))
        return handleCryptoErrors();
